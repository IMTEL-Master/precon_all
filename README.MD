...
1. **FreeSurfer License**: Required for operation
   ...
   - Save as `license.txt` in the 
      - Bare-metal: your `FREESURFER_HOME` directory (downloaded later)
      - Docker: project root
...
1. **Clone Repository**:
   ```bash
   git clone https://github.com/IMTEL-Master/precon_all.git
   cd precon_all
   ```
...
2. **Download and Install Dependencies**:
   ...
   The `predownload-dependencies.sh` script:
   ...
   Platform Notes:
   - Linux: Fully automated installation
   - macOS: Guided installation using brew or manual paths
   - Windows: Run inside WSL 2 (Ubuntu) for compatibility; native install not supported
...
3. **Set Environment Variables**:
   Add these exports to your shell configuration:
   - Linux/macOS → add to ~/.bashrc or ~/.zshrc
   - Windows (WSL) → add to ~/.bashrc   
   ```
   # These are general paths, make sure they point to the correct location
   export PCP_PATH=~/precon_all
   export FREESURFER_HOME=~/freesurfer-6.0.0
   export ANTSPATH=~/ants-2.4.3/bin
   export FSLDIR=~/fsl
   export WORKBENCH=~/workbench-1.5.0/workbench/bin
   export PATH=$PCP_PATH/bin:$FREESURFER_HOME:$ANTSPATH:$FSLDIR/bin:$WORKBENCH:$PATH
   ```
   Apply changes:
   ```bash
   source ~/.bashrc
...
5. **Run Container**:
   ...
   docker run -it \
     ...
     precon_all bash
   ```
   Windows users: Replace $(pwd) with %cd% in PowerShell or CMD.
...
#### Using ITK-SNAP:
1. Open your T1 image in ITK-SNAP
2. Activate the paintbrush tool: Tools → Active Main Tool → Paint Brush Tool
3. For each required mask:
   - Create a new label or use existing labels
   - Navigate through slices and manually paint the appropriate brain regions
   - Use different labels for different mask types (e.g., label 1 for left hemisphere, label 2 for right)
4. Save each mask separately: File → Save Data → Segmentation Data
5. Ensure saved files are named correctly (e.g., brain_mask.nii.gz)

#### Alternative Methods:
- Extract masks from existing parcellations (e.g., FreeSurfer, FSL FAST)
- Use automated segmentation tools specific to your species
- Adapt atlases from similar species if available

#### Quality Check: Before proceeding, verify that:
- All masks are binary (only 0s and 1s)
- Masks have the same dimensions and orientation as your T1 image
- Left and right hemisphere masks don't overlap
- The brain mask fully encompasses all other masks
